    ...
    if clicks:
        if contents is not None:
            ...
            if tabs =='summary':
                ...
                name,value1 = metrics(weight,price,Benchmark)
                value = [ '%.4f' % elem for elem in value1 ]
                dfm=pd.DataFrame()
                dfm['Names']=name
                dfm['{}'.format(goal)]=value
                wP = list(yourAllo['Provided Allocation'])
                nameP,valueP1 = metrics(wP,price,Benchmark)
                valueP = [ '%.4f' % elem for elem in valueP1 ]
                ...
                'Provided Protfolio Returns(%)':old_returns}
                df_MR = DataFrame.from_dict(dic)
                dic_cum = {'Dates':dates,'Optimized Cum Returns(%)':pf_cum,
                'Provided Cum Returns(%)':old_cum}
                df_cum = DataFrame.from_dict(dic_cum)
                return html.Div([
                html.Div([
                html.Div([
                    dash_table.DataTable(
                    ...
                    html.Div([
                        dash_table.DataTable(
                            ...
                            columns=[{"name": i, "id": i} for i in df_cum.columns]
                        ),],className='col-md-5'),
                        html.Div([
                        dcc.Graph(id='CumGraph',
                                     figure={
                                        'data':[
                                                go.Scatter(
                                                            x = dates,
                                                            y = pf_cum,
                                                            mode = 'lines+markers',
                                                            name = 'Optimized Annual Cum Return'),
                                                go.Scatter(
                                                            x = dates,
                                                            y = old_cum,
                                                            mode = 'lines+markers',
                                                            name = 'Provided Annual Cum Return')
                                                    ],
                                         'layout': go.Layout(yaxis=go.layout.YAxis(title='Portfolio Historical Cum Returns (%)'),
                                        xaxis=go.layout.XAxis(title='Dates'))
                                     })
                        ],className='col-md-7')
                        ],className='row')
                                     ...
                                     figure={
                                        'data':[
                                                go.Scatter(
                                                            ...
                                                            y = pf_cum,
                                                            mode = 'lines+markers',
                                                            name = 'Optimized Monthly Cum Return'),
                                                go.Scatter(
                                                            x = dates,
                                                            y = old_cum,
                                                            mode = 'lines+markers',
                                                            name = 'Provided Monthly Cum Return')
                                                    ],
                                         'layout': go.Layout(yaxis=go.layout.YAxis(title='Portfolio Historical Cum Returns (%)'),
                                        xaxis=go.layout.XAxis(title='Dates'))
                                     })
